...
export function nodeReducer(node: Node, action: Exclude<SlateOperation, SetSelectionOperation>): Node {
...
function applyNodeReducerToChildElement<T extends Node>(node: T, action: Exclude<SlateOperation, SetSelectionOperation>): T {
    if (Node.isElement(node) && 0 <= action.path[0] && action.path[0] < node.children.length) {
        return ({
            ...node,
            children: [
